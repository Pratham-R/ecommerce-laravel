...
class AdminController extends Controller
{
    ...
    public function rejectProduct($id)
    {
        $product = Product::findOrFail($id);
        $product->delete();
        return redirect()->back()->with('success', 'Product rejected and deleted successfully');
    }

    public function sellers()
    {
        $sellers = User::where('role', 'seller')
                      ->withCount(['products', 'products as pending_products_count' => function($query) {
    ...
    public function showSeller($id)
    {
        ...
        $pending_products = $seller->products()
                                 ->where('is_approved', false)
                                 ->with('category')
                                 ->paginate(10);

        return view('admin.sellers.show', compact('seller', 'pending_products'));
    }
